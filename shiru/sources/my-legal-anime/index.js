...
export default new class MyLegalAnime extends AbstractSource {
  ...
  async single({ titles }) {
    ...
    const search = await this.request(
      ...
      '&limit=3&sort_by=seeders'
    )

    if (search.error) {
      if (search.retry_after) {
        return this.makeDebug('RATE LIMITED', String(search.retry_after) + 's')
      }
    ...
    if (!rows.length) {
      return this.makeDebug('NO MATCHES', title)
    }
    ...
    if (!results.length) {
      return this.makeDebug('NO MAGNET IN SEARCH', title)
    }
