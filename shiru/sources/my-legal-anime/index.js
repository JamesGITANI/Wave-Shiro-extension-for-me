...
export default new class MyLegalAnime extends AbstractSource {
  ...
  shortenTitle(text = '') {
    ...
    return t
      .replace(/\b(episode|season|part|cour)\b.*$/i, '')
      .replace(/\b(2nd season|3rd season|season \d+)\b/ig, '')
      .replace(/\s+/g, ' ')
      .trim()
  }
  ...
  getTitles(input) {
    ...
    for (const t of raw) {
      for (const variant of this.titleVariants(t)) {
  ...
  async searchTorrentsByText(title) {
    const qs = new URLSearchParams({
      query: title,
      sort_by: 'seeders',
      limit: '25'
  ...
  async collectDetailedResults(rows) {
    const results = []
  ...
  async single(options) {
    ...
    for (const title of titles) {
      ...
      for (const media of mediaRows.slice(0, 5)) {
        ...
        const results = await this.collectDetailedResults(rows)
        if (results.length) return results
      }
    }

    for (const title of titles) {
      const directSearch = await this.searchTorrentsByText(title)

      if (directSearch.error) {
        return [this.makeDebug('DIRECT SEARCH FAILED', directSearch.message || 'unknown')]
      }

      const rows = Array.isArray(directSearch.data) ? directSearch.data : []
      if (!rows.length) continue
